(async () => {
  ...
  const render = (car) => {
     ...
     * --------------------------------------------------
     * FIX:
     * Previously the values were wrapped in <span>
     * BEFORE being passed through list(), which escaped
     * the HTML and displayed the <span> tags as text.
     *
     * Now the actual text is escaped first and then
     * safely wrapped in a real <span>.
     */
    ...
    if (!bestFor.children.length) {
      bestFor.innerHTML = '<span>Not listed</span>';
    }
        ...
        .map(
          ([k, v]) =>
            `<div class="spec-row">
              ...
              <strong>${esc(v || 'Not listed')}</strong>
            </div>`
        )
        .join('');
          ...
          .map(
            (img, i) =>
              ...
              <button
                ...
                data-index="${i}"
                data-src="${esc(imageUrl(img))}"
                type="button"
              >
                <img
                  src="${esc(imageUrl(img))}"
          ...
          btn.addEventListener('click', () => {
            ...
            setTimeout(() => {
              ...
              $('galleryCaption').textContent =
                `${String(
                  Number(btn.dataset.index) + 1
                ).padStart(2, '0')} / EXTERIOR`;
            }, 140);
          });
        });
    } else {
